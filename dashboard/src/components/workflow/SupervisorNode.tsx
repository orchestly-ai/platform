...
import { Crown, DollarSign, Clock, FileText } from 'lucide-react'
import { AgentNodeData } from '../../types'

function SupervisorNode({ data, selected }: NodeProps<AgentNodeData>) {
  const statusColors = {
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <div className="flex-1">
          <div className="text-xs font-medium text-purple-900">SUPERVISOR</div>
        ...
        {(data.modelSelection || data.llmModel) && (
          <div className="flex items-center gap-1">
            ...
            {data.modelSelection === 'auto' && (
              <span className="truncate text-purple-600 font-medium">Auto (Routing)</span>
            )}
        ...
        {data.capabilities && data.capabilities.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-1">
            {data.capabilities.slice(0, 2).map((cap, idx) => (
              <span key={idx} className="px-1.5 py-0.5 bg-purple-200 text-purple-900 rounded text-xs">
                {cap}
              </span>
            ))}
